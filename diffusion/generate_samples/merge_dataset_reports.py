...
if __name__ == "__main__":
    ...
    csv_save_folder = csv_save_folder = os.path.join(save_folder, f"csvs")
    file_names = [os.path.join(csv_save_folder, f"report_{c}.csv") for c in range(args.chunks)]
    ...
    df = pd.concat(dfs)
    final_path = os.path.join(csv_save_folder, f"report_{timestamp}.csv")
